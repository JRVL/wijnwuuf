 <!-- Font awesome import -->
<link rel="stylesheet" href="//netdna.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">



<body>



<div class="footer">
  <div class="colfooter column1">
       <div class="footer-content">
      
          <a href="<?php echo esc_url( home_url( '/' ) ); ?>"><img src="<?php bloginfo ('template_url') ?>/images/logo.svg" alt="" class="logowit"></a>
      
     </div></div>
    
    
  <div class="colfooter column2">
      <div class="footer-content">
    <h1 class="footer-title">Welkom</h1>
      <br><p class="footer-text">Wat leuk dat je mijn blog leest! Je hebt het vast al gemerkt.. ik ben dól op wijn en combineer deze passie met mijn passie voor schrijven en fotografie. Meer over mij? Lees dan snel verder! </p>
      <a id="readmorebutton1" style="padding: 0.45em 1.8em 0.45em 1.8em; font-size: 12px; margin: auto; margin-right: 15px;">Over mij</a><a id="readmorebutton1" style="padding: 0.45em 1.8em 0.45em 1.8em; font-size: 12px; margin: auto;">Proeverij</a>
  </div></div>
    
    <div class="colfooter column3">
    <div class="footer-content">
        <h1 class="footer-title">Contact</h1>
        <h2 class="footer-text">
            <p class="footer-text"><br>Amber Lukaart<br>
            <a class="footer-text" style="color:#000" href="mailto:info@wijnwuuf.nl">info@wijnwuuf.nl</a><br><br>
                <i style="margin-right: 10px;"  class="icon fab fa-instagram"></i>
                <i class="icon fab fa-facebook"></i></p>
        </h2></div>
        
    </div></div>
      

<div class="footer2">&copy; Wijnwuuf 2019 - Privacyverklaring</div>
    
   

</body>